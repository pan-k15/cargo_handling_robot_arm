#!/usr/bin/env python3
"""
sort_orchestrator — pick-and-place node for a single green cube.

Flow
----
1. (optional, use_camera=true) Move arm to scan pose above the cube and
   wait up to 5 s for /object_class to confirm the object is present.
2. Call /pick  to grasp the cube at _OBJECT.
3. Call /place to drop the cube at _DROP.

Object position  (world frame, from object.sdf):
  green cube: (0.50, 0.00, 0.15)
...
# Single object and its drop position
_OBJECT = (0.50, 0.00, 0.15)   # green cube centre (x, y, z)
_DROP   = (0.35, 0.50, 0.07)   # green bin landing position
...
class PickOrchestrator(Node):
    ...
    def __init__(self):
        ...
        # When true: move to scan pose and log /object_class before picking.
        # When false (default): go straight to pick.
    ...
    def _delayed_start(self):
        time.sleep(6.0)
        self.get_logger().info('Pick orchestrator: starting …')
        self._run_sequence()
    ...
    def _run_sequence(self):
        ...
        if self.get_parameter('use_camera').get_parameter_value().bool_value:
        ...
        self.get_logger().info(f'Picking from ({ox}, {oy}, {oz}) …')
        if not self._do_pick(ox, oy, oz):
            self.get_logger().error('Pick failed — aborting')
            return

        self.get_logger().info(f'Placing at ({dx}, {dy}, {dz}) …')
