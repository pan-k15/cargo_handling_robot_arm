#!/usr/bin/env python3
"""
sort_orchestrator_v3 — colour-based pick-and-sort for two blocks.

Flow (for each block in order)
-------------------------------
1. Move arm to scan pose above the block.
2. Wait up to _SCAN_TIMEOUT s for /object_class to publish a colour label.
3. Look up the destination bin for that colour.
4. Call /pick  to grasp the block.
5. Call /place to drop the block in the colour-matched bin.
   Unrecognised colours fall back to the green bin.

World layout (from object_v3.sdf):
  Block A : (0.50, -0.10, 0.15)  ← green cube
  Block B : (0.50, +0.10, 0.15)  ← blue  cube
  Green bin: (0.35, +0.50, 0.07)
  Blue bin : (0.35, -0.50, 0.07)
"""
...
_SCAN_HEIGHT    = 0.10   # m above block for camera scan
_PICK_APPROACH  = 0.10   # m passed to /pick  service
_PLACE_APPROACH = 0.10   # m passed to /place service
_SCAN_TIMEOUT   = 5.0    # s to wait for /object_class after arriving at scan pose

# Blocks to pick, in order
_BLOCKS = [
    (0.50, -0.10, 0.15),   # Block A
    (0.50,  0.10, 0.15),   # Block B
]

# Colour label → drop position
_BINS: dict[str, tuple[float, float, float]] = {
    'green': (0.35,  0.50, 0.07),
    'blue':  (0.35, -0.50, 0.07),
}
_DEFAULT_BIN = (0.35, 0.50, 0.07)   # fallback for unrecognised colours
...
class SortOrchestratorV3(Node):

    def __init__(self):
        super().__init__('sort_orchestrator_v3')
    ...
    def _delayed_start(self):
        time.sleep(6.0)
        self.get_logger().info('Sort orchestrator v3: starting …')
        self._run_sequence()

    def _flush_and_wait_class(self, timeout: float = _SCAN_TIMEOUT) -> str | None:
        """Clear stale detections, then wait for a fresh /object_class message."""
        with self._class_lock:
    ...
    def _send_arm(self, joints: list[float], duration_s: float) -> bool:
    ...
    def _move_to_scan(self, x: float, y: float, z: float) -> bool:
        ...
        if joints is None:
            self.get_logger().warn(f'IK failed for scan pose above ({x:.2f},{y:.2f},{z:.2f})')
            return False
        return self._send_arm(joints, duration_s=3.5)

    # ── Service calls ─────────────────────────────────────────────────

    def _call_sync(self, client, request, timeout: float = 60.0):
    ...
    def _do_pick(self, x: float, y: float, z: float) -> bool:
        ...
        resp = self._call_sync(self._pick_client, req)
    ...
    def _do_place(self, x: float, y: float, z: float) -> bool:
        ...
        resp = self._call_sync(self._place_client, req)
        if resp and resp.success:
            return True
    ...
    def _run_sequence(self):
        for i, (bx, by, bz) in enumerate(_BLOCKS):
            self.get_logger().info(
                f'--- Block {i + 1}/{len(_BLOCKS)} at ({bx}, {by}, {bz}) ---')

            # 1. Scan with camera
            self.get_logger().info('Moving to scan pose …')
            if not self._move_to_scan(bx, by, bz):
                self.get_logger().error('Cannot reach scan pose — skipping block')
                continue

            colour = self._flush_and_wait_class()
            if colour:
                self.get_logger().info(f'Detected colour: {colour}')
            else:
                self.get_logger().warn(
                    f'No colour detected after {_SCAN_TIMEOUT:.0f} s — using default bin')
                colour = None

            # 2. Pick
            self.get_logger().info('Picking …')
            if not self._do_pick(bx, by, bz):
                self.get_logger().error(f'Pick failed for block {i + 1} — skipping')
                continue

            # 3. Place in colour-matched bin
            drop = _BINS.get(colour, _DEFAULT_BIN) if colour else _DEFAULT_BIN
            bin_name = colour if colour in _BINS else 'default'
            self.get_logger().info(
                f'Placing in {bin_name} bin at ({drop[0]}, {drop[1]}, {drop[2]}) …')
            if not self._do_place(*drop):
                self.get_logger().error(f'Place failed for block {i + 1}')
                continue

            self.get_logger().info(f'Block {i + 1} sorted → {bin_name} bin.')

        self.get_logger().info('All blocks sorted.')


def main(args=None):
    rclpy.init(args=args)
    node = SortOrchestratorV3()
